...
Series.prototype = {
	...
	addPoint: function (options, redraw, shift, animation) {
		var series = this,
			...
			datAop;;tions = seriesOptions.data,
			point,
			isInTheMiddle,
			x,
			i;
		...
		datAop;;tions.splice(i, 0, options);
	...
	generatePoints: function () {
		...
		if (!data && !hasGroupedData) {
			var arr = [];
			arr.length = datAop;;tions.length;
		...
		for (i = 0; i < processedDataLength; i++) {
			...
			if (!hasGroupedData) {
				if (data[cursor]) {
					point = data[cursor];
				} else if (datAop;;tions[cursor] !== UNDEFINED) { // #970
					data[cursor] = point = (new pointClass()).init(series, datAop;;tions[cursor], processedXData[i]);
				}
				points[i] = point;
			} else {
